...
scriptName = 'pharmacophore_script.py'
...
class ProtocolPharmacophoreFiltering(EMProtocol):
    ...
    def writeParamsFile(self, paramsFile, ligands, molsScipion):
        ...
        for ligand in ligands:
            ligandsFiles.append(os.path.abspath(ligand.getFileName()))
            smile = ligand.getSmiles()
            id = ((ligand.getFileName()).split("/")[-1]).replace("_lig.pdb", "")
